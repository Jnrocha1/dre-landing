...
const ITEMS = [
  { icon: "⏱", title: "2 a 4 horas por empresa por mês", desc: "Copiando valores de PDF para planilha célula por célula." },
  { icon: "📊", title: "Gráficos que o cliente não entende", desc: "Planilha de Excel não tem impacto. O dono não sabe o que fazer com os números." },
  { icon: "💸", title: "Honorários que não crescem", desc: "Mais clientes = mais digitação. Os honorários não refletem o valor entregue." },
]
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "3rem", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem" }}>
            {ITEMS.map((item, i) => (
              <motion.div key={item.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12 }}
                ...
                <div style={{ width: 34, height: 34, borderRadius: 8, background: "rgba(255,61,87,.1)", border: "1px solid rgba(255,61,87,.18)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 16 }}>{item.icon}</div>
                <div>
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
          <div style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,61,87,.18)", borderRadius: 14, padding: "1.15rem" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "var(--red)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "0.8rem" }}>⚠ Antes do DRE Analytics</div>
            ...
            <div style={{ padding: 8, background: "rgba(255,61,87,.07)", borderRadius: 6, fontSize: 11, color: "var(--red)", textAlign: "center", fontWeight: 700, marginTop: "0.25rem" }}>
              ⏱ 3 horas por empresa · todo mês · sem fim
            </div>
          </div>
        </motion.div>
      </div>
